...
struct freeze_info {
    ...
    uint64_t start_delay;   // kept as ms (integer) for simplicity in UI
    uint64_t end_delay;     // kept as ms (integer) for simplicity in UI
...
static void freeze_update(void* data, obs_data_t* settings)
{
    ...
    freeze->duration_max = (float)obs_data_get_double(settings, "duration");
    freeze->refresh_interval = (float)obs_data_get_double(settings, "refresh_interval");
    freeze->start_delay = (uint64_t)obs_data_get_double(settings, "start_delay");
    freeze->end_delay = (uint64_t)obs_data_get_double(settings, "end_delay");
...
static obs_properties_t* freeze_properties(void* data)
{
    ...
    p = obs_properties_add_float(ppts, "duration", obs_module_text("Duration"), 0.0, 100000.0, 0.1);
    obs_property_float_set_suffix(p, " ms");

    p = obs_properties_add_float(ppts, "refresh_interval", obs_module_text("RefreshInterval"), 0.0, 100000.0, 0.1);
    obs_property_float_set_suffix(p, " ms");

    p = obs_properties_add_float(ppts, "start_delay", obs_module_text("StartDelay"), 0.0, 100000.0, 0.1);
    obs_property_float_set_suffix(p, " ms");

    p = obs_properties_add_float(ppts, "end_delay", obs_module_text("EndDelay"), 0.0, 100000.0, 0.1);
    obs_property_float_set_suffix(p, " ms");
    ...
    p = obs_properties_add_float_slider(group, "mask_left", obs_module_text("MaskLeft"), 0.0, 100.0, 0.01);
    obs_property_float_set_suffix(p, "%");
    p = obs_properties_add_float_slider(group, "mask_right", obs_module_text("MaskRight"), 0.0, 100.0, 0.01);
    obs_property_float_set_suffix(p, "%");
    p = obs_properties_add_float_slider(group, "mask_top", obs_module_text("MaskTop"), 0.0, 100.0, 0.01);
    obs_property_float_set_suffix(p, "%");
    p = obs_properties_add_float_slider(group, "mask_bottom", obs_module_text("MaskBottom"), 0.0, 100.0, 0.01);
    obs_property_float_set_suffix(p, "%");
    obs_properties_add_group(ppts, "mask", obs_module_text("Mask"), OBS_GROUP_CHECKABLE, group);

    // Action group
    group = obs_properties_create();
    p = obs_properties_add_list(group, "activate_action", obs_module_text("ActivateAction"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
    prop_list_add_actions(p);
    p = obs_properties_add_list(group, "deactivate_action", obs_module_text("DeactivateAction"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
    prop_list_add_actions(p);
...
{
    obs_data_set_default_double(settings, "duration", 5000.0);
    obs_data_set_default_double(settings, "refresh_interval", 0.0);
    obs_data_set_default_double(settings, "start_delay", 0.0);
    obs_data_set_default_double(settings, "end_delay", 0.0);
    obs_data_set_default_bool(settings, "log_enable", false);
...
void freeze_do_or_delay_action(struct freeze_info* freeze, uint32_t action)
{
    ...
    if ((freeze->start_delay > 0 && action == FREEZE_ACTION_ENABLE) ||
        (freeze->end_delay > 0 && action == FREEZE_ACTION_DISABLE)) {

        if (freeze->log_enable) {
            obs_source_t* parent = obs_filter_get_parent(freeze->source);
            blog(LOG_INFO, "[Freeze Filter] %s Delayed '%s'",
                action == FREEZE_ACTION_ENABLE ? "Enable" : "Disable",
                parent ? obs_source_get_name(parent) : obs_source_get_name(freeze->source));
        }
...
// ... (hotkey functions remain unchanged) ...
...
bool freeze_enable_hotkey(void* data, obs_hotkey_pair_id id, obs_hotkey_t* hotkey, bool pressed)
{
    ...
    struct freeze_info* freeze = data;
    if (!pressed || obs_source_enabled(freeze->source))
        return false;
...
static void freeze_tick(void* data, float t)
{
    ...
    if (f->delayed_action != FREEZE_ACTION_NONE) {
        f->delay_duration += t;
        uint64_t required_delay = (f->delayed_action == FREEZE_ACTION_ENABLE) ? f->start_delay : f->end_delay;
        if (f->delay_duration * 1000.0f >= (float)required_delay) {
    ...
    if (obs_source_enabled(f->source)) {
        if (f->log_enable && f->duration == 0.0f) {
            ...
            blog(LOG_INFO, "[Freeze Filter] Enabled '%s'",
                parent ? obs_source_get_name(parent) : obs_source_get_name(f->source));
        }

        f->duration += t;

        if (f->duration_max > 0.0f && f->duration * 1000.0f > f->duration_max) {
            obs_source_set_enabled(f->source, false);
        }
        else if (f->refresh_interval > 0.0f && f->duration > f->last_refresh &&
            (f->duration - f->last_refresh) * 1000.0f >= f->refresh_interval) {
        ...
        if (f->log_enable && f->duration != 0.0f) {
            obs_source_t* parent = obs_filter_get_parent(f->source);
            blog(LOG_INFO, "[Freeze Filter] Disabled '%s'",
                parent ? obs_source_get_name(parent) : obs_source_get_name(f->source));
        }
        f->processed_frame = false;
        f->duration = 0.0f;
